...
Pauses every N tasks for user review instead of running perpetually. Set `LOKI_AUTONOMY_MODE=checkpoint` and `LOKI_CHECKPOINT_FREQUENCY=10`. Orchestrator writes `.loki/signals/CHECKPOINT_SUMMARY_*.md`, waits for `.loki/signals/CHECKPOINT_APPROVED`. See `agent-skills/checkpoint-mode/README.md`.
